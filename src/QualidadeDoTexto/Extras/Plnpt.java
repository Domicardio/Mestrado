 ...
 */
package QualidadeDoTexto.Extras;
...
public class Plnpt {
    ...
    public Plnpt() {
        ...
        modelFileName[STDETECT]=  "pt-sent.bin";
        modelFileName[TOKENIZER]= "pt-token.bin";
        modelFileName[TAGGER]=    "pt-pos-maxent.bin";
        modelFileName[CHUNKER]=   "en-chunker.bin";
        modelFileName[PARSER]=    "pt-parser-chunking.bin";
    }
     ...
     * @param stc Sentence The sentence to be tagged.
     * @return Sentence The tagged sentence.
     */
    public Sentence postag(Sentence stc) {
        if ( stc == null || !configurationOK(TAGGER) )  return null;
        
        String[] vs= postag(stc.toString()).split(" ");

        stc.clear();
        for (int k=0; k<vs.length; k++) {
            int r= vs[k].lastIndexOf('/');
            if ( r == -1 ) {
                stc.addWord(new Word(vs[k], "UNK"));
                continue;
            }

            String token= vs[k].substring(0,r);
            String   tag= vs[k].substring(r+1);
            stc.addWord(new Word(token, tag));
        }
    ...
    public static void main(String[] args) {
        //OpenNLPKit model = new OpenNLPKit("/a/tools/opennlp-tools-1.5.0/models/english/");
        Plnpt model = new Plnpt(HULTIGVars.PATH_OPENLP+ "/models/english/");
